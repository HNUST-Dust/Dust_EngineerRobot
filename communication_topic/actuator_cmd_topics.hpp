...
// ===================== CubeMars MIT-like =====================
// 说明：沿用 MIT 打包格式（p/v/kp/kd/t -> 8 bytes），但发送 std_id/接收帧格式由 CubeMars 驱动约定。
struct CubemarsTargetCmd {
    CanBus bus = CanBus::CAN1;
    uint8_t can_rx_id = 0x01;
...
inline RingTopic<CubemarsTargetCmd, 16> cubemars_target_cmd;

enum class CubemarsAdminOp : uint8_t {
    Enter = 0,
    Exit,
    SaveZero,
};

struct CubemarsAdminCmd {
    CanBus bus = CanBus::CAN1;
    uint8_t can_rx_id = 0x01;
    CubemarsAdminOp op = CubemarsAdminOp::Enter;
};

inline RingTopic<CubemarsAdminCmd, 8> cubemars_admin_cmd;
