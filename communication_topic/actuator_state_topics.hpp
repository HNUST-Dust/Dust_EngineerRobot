...
// ===================== CubeMars MIT-like feedback =====================
// 来自 CubeMars 反馈帧解包（p16 v12 t12），在驱动内做角度 unwrap 得到 total_angle。
struct CubemarsFeedback {
    ...
    // 预留：若后续协议包含温度/错码，可在此扩展
};

constexpr int cubemars_feedback_index(CanBus bus, uint8_t can_rx_id)
{
    const int bus_i = static_cast<int>(bus);
    if (bus_i < 1 || bus_i > 3) {
        return (can_rx_id & 0x0F);
...
inline MultiTopic<CubemarsFeedback, 48> cubemars_feedback;
