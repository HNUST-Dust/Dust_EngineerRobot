...
void Chassis::Exit()
{
    ...
    static constexpr uint16_t kIds[4] = {
        static_cast<uint16_t>(motor_ids::kMotorChassis1),
        static_cast<uint16_t>(motor_ids::kMotorChassis2),
        static_cast<uint16_t>(motor_ids::kMotorChassis3),
        static_cast<uint16_t>(motor_ids::kMotorChassis4),
    };
...
void Chassis::Task()
{
    ...
    for (;;)
    {
        ...
        const float target_vx_in_chassis_ = (rc_control.chassis_speed_x - 127.0f) * kChassisSpeed / 128.0f;
        const float target_vy_in_chassis_ = (127.0f - rc_control.chassis_speed_y) * kChassisSpeed / 128.0f;
        float target_velocity_rotation_ = (127.0f - rc_control.chassis_rotation) * kChassisSpeed / 128.0f;
