...
static void dm_mit_task(void*) {
    ...
    // Always subscribe both IMU sources; runtime selects per motor/per loop.
    Subscription<orb::ImuData> imu_local_sub(orb::imu_data);
    Subscription<orb::McuImu> imu_mcu_sub(orb::mcu_imu);
    orb::ImuData imu_local{};
    orb::McuImu imu_mcu{};

    for (;;) {
        // 最新 IMU（用于云台级串级 PID 反馈）
        (void)imu_local_sub.copy(imu_local);
        (void)imu_mcu_sub.copy(imu_mcu);
        ...
        // 串级 PID 计算与下发：对 active motors 按每电机 control_rate_hz 调度
        ...
        for (auto &e : s_active_cascade) {
            ...
            // Per-motor schedule gate
            const uint32_t period = m->ctrl_period_ticks();
            if (period != 0) {
                if (e.next_ctrl_tick == 0) {
                    // first run immediately
                    e.next_ctrl_tick = now_tick;
                }
                // handle tick wrap with signed subtraction
                if (static_cast<int32_t>(now_tick - e.next_ctrl_tick) < 0) {
                    continue;
                }
                e.next_ctrl_tick = now_tick + period;
            }

            // Fetch encoder feedback on demand
            const int fb_idx = orb::dm_mit_feedback_index(m->bus(), m->can_rx_id() & 0x0F);
            Subscription<orb::DmMitFeedback> fb_sub(orb::dm_mit_feedback[fb_idx]);
            ...
            // Angle feedback source (per motor)
            switch (m->angle_feedback()) {
            case DmMitMin::CascadeFeedback::Encoder:
                measured_angle = fb.total_angle_rad;
                break;
            case DmMitMin::CascadeFeedback::ImuLocal:
                measured_angle = (m->cascade_axis() == DmMitMin::CascadeAxis::Yaw) ? imu_local.yaw_angle : imu_local.pitch_angle;
                break;
            case DmMitMin::CascadeFeedback::ImuMcu:
                measured_angle = (m->cascade_axis() == DmMitMin::CascadeAxis::Yaw) ? imu_mcu.yaw_total_angle_f : imu_mcu.pitch_f;
                break;
            default:
                measured_angle = fb.total_angle_rad;
                break;
            ...
            // Omega feedback source (per motor)
            switch (m->omega_feedback()) {
            case DmMitMin::CascadeFeedback::Encoder:
                measured_omega = fb.omega_rad_s;
                break;
            case DmMitMin::CascadeFeedback::ImuLocal:
                measured_omega = (m->cascade_axis() == DmMitMin::CascadeAxis::Yaw) ? imu_local.yaw_omega : imu_local.pitch_omega;
                break;
            case DmMitMin::CascadeFeedback::ImuMcu:
                measured_omega = (m->cascade_axis() == DmMitMin::CascadeAxis::Yaw) ? imu_mcu.yaw_omega_f : imu_mcu.pitch_omega_f;
                break;
            default:
                measured_omega = fb.omega_rad_s;
                break;
            ...
            // PublishMitTx() 内部仍保留调度保护（冗余防御），但控制计算已按频率统一
...
void DmMitMin::Init(BspCanHandle can, const Config& cfg) {
    ...
    // Per-motor control rate limiting (control frequency == CAN command publish frequency)
    last_ctrl_tick_ = 0;
    ctrl_period_ticks_ = 0;
    if (cfg_.control_rate_hz > 0) {
        const uint32_t tick_hz = osKernelGetTickFreq();
        // ceil(tick_hz / hz) to avoid faster-than-requested control
        ctrl_period_ticks_ = (tick_hz + static_cast<uint32_t>(cfg_.control_rate_hz) - 1u) / static_cast<uint32_t>(cfg_.control_rate_hz);
        if (ctrl_period_ticks_ == 0) {
            ctrl_period_ticks_ = 1;
        }
    }
...
void DmMitMin::PublishMitTx(float kp, float kd) {
    // Control scheduling / rate limiting
    if (ctrl_period_ticks_ != 0) {
        const uint32_t now = osKernelGetTickCount();
        if ((now - last_ctrl_tick_) < ctrl_period_ticks_) {
            return;
        }
        last_ctrl_tick_ = now;
    }
...
{
    alg::PidConfig a = angle_cfg;
    alg::PidConfig o = omega_cfg;

    // Auto-derive PID dt from per-motor control frequency.
    // Note: if control_rate_hz==0, keep dt provided by caller (typically 1ms defaults).
    if (cfg_.control_rate_hz > 0) {
        const float dt = 1.0f / static_cast<float>(cfg_.control_rate_hz);
        a.dt = dt;
        o.dt = dt;
    }

    pid_angle_.configure(a);
    pid_omega_.configure(o);
...
actuator::drivers::DmMitMin motor_x_axis_right{};
actuator::drivers::DmMitMin motor_x_axis_left{};
actuator::drivers::DmMitMin elbow_joint_pitch{};
actuator::drivers::DmMitMin elbow_joint_yaw{};
actuator::drivers::DmMitMin claws{};
} // namespace actuator::instances
