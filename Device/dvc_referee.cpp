...
{
    referee::RefereeUartData *tmp_buffer;

    for (int i = 0; i < length;)
    {
        tmp_buffer = (referee::RefereeUartData *) &buffer[i];

        // 未通过头校验
        if (tmp_buffer->frame_header.sof != 0xA5)
        {
            i++;
            continue;
        }
        // 未通过CRC8校验, 顺一位继续判断
        // if (verify_crc8_check_sum((uint8_t *) tmp_buffer, 4) != tmp_buffer->frame_header.crc8)
        // {
        //     i++;
        //     continue;
        // }
        ...
        // }
        // 通过校验但帧不够长
        if (i + 7 + tmp_buffer->frame_header.data_length + 2 > length)
        {
            break;
        }
        switch(tmp_buffer->referee_command_id)
        {
            case referee::kStatusDataId:
            {
                UBaseType_t uxSavedInterruptStatus = taskENTER_CRITICAL_FROM_ISR();
                std::memcpy(&status_, tmp_buffer->data, sizeof(referee::StatusData));
                taskEXIT_CRITICAL_FROM_ISR(uxSavedInterruptStatus);
                break;
            }
            case referee::kShootDataId:
            {
                UBaseType_t uxSavedInterruptStatus = taskENTER_CRITICAL_FROM_ISR();
                std::memcpy(&shoot_, tmp_buffer->data, sizeof(referee::StatusData));
                taskEXIT_CRITICAL_FROM_ISR(uxSavedInterruptStatus);
                break;
            }
            case referee::kGameStatusId:
            {
                UBaseType_t uxSavedInterruptStatus = taskENTER_CRITICAL_FROM_ISR();
                std::memcpy(&game_status_, tmp_buffer->data, sizeof(referee::ShootData));
                taskEXIT_CRITICAL_FROM_ISR(uxSavedInterruptStatus);
                break;
            }
            default:
        ...
        // 缓冲区直接推移
        i += 7 + tmp_buffer->frame_header.data_length + 2;
    }
}
